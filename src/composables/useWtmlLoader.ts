import {ref, watch, type Ref, computed, shallowRef} from "vue";
...
export interface WtmlLoaderReturn {
  ready: Promise<void>;
...
export function useWtmlLoader(wtmlUrl: string, _options?: WtmlLoaderOptions): Prettify<WtmlLoaderReturn> {
  ...
  const imagesets = shallowRef<Imageset[]>([]);
  const imagesetLayers = shallowRef<ImageSetLayer[]>([]);
  const fitsImages = shallowRef<(FitsImage | null)[]>([]);
